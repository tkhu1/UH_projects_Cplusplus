...
class BTree
{
    ...
    {  
		...
		{
			//init output stream
			ofstream outfs("output.txt", ios_base::app);
		...
		void levelTraverse(int inputLevel) 
		{
			//init output stream
			ofstream outfs("output.txt", ios_base::app);
...
int main(int argc, char* argv[]) {
	...
	*/
	...
	//init input stream
	ifstream infs("input80.txt");

	//init command stream
	ifstream commfs("command80.txt");
		...
		if (!temp.empty() && temp != "\r") {
			...
			if (tempSize != 0) {
				...
				if (temp == "Inorder Traversal\r") {
					...
					commLevel >> tempLevel; 
					myTree.levelTraverse(tempLevel);
