      ...
      //this is a child process
                  ...
                  for (int k = 0; k < totalProcesses; k++) {
                     ...
                     if (instructionProcessIDtoString.find(processIDtoString) != string::npos) {
                        ...
                        if (PageFault) {
                           ...
                           //searches if the page already exists, then removes it from head
                           for (int n = 0; n < LRU_Array.size(); n++)
                              if (LRU_Array[n]->pageID == ptrForPage->pageID)
                              {
                                 LRU_Array.erase(LRU_Array.begin());
                                 break;
                              }

                           //pushes the new page pointer
                           LRU_Array.push_back(ptrForPage);
            ...
            if (i == 2) {
               ...
               while (running == 1) {
                  ...
                  ptrForFrame = &framesInMainMemory[LRU_Array[0]->frameID];

                  //	remove head from LRU list
                  LRU_Array.erase(LRU_Array.begin());
